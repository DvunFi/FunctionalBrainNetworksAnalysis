library(igraph)
i<-5
prefix<-paste("m",i,"c0_V_BC_q=0",sep = "")
path<-paste("D:/VKR/VKRdocs/scripts/data/1000/Results/targetAttacsVer/For1000/m=5/BC/c4/",sep = "")
delet_count<-10
...
# plot(data_frame_for_max_com_and_k[[1]],data_frame_for_max_com_and_k[[3]],xlab="удалено вершин", 
...
# dev.off ();
# plot(data_frame_for_max_com_and_k[[9]],data_frame_for_max_com_and_k[[8]],xlab="дол€ удаленных вершин", 
#      ylab="количество св€зий",main="количество св€зий от удалени€ вершин")
# dev.copy(jpeg,filename=paste(path,"/количество св€зий от удалени€ вершин(направ v1000",prefix,")",".jpg",sep = ""));
# dev.off ();
plot(data_frame_for_max_com_and_k[[9]],data_frame_for_max_com_and_k[[2]] ,xlab="дол€ удаленных вершин", 
     ylab="дол€ вершин принадлежащих ћ— ", xlim=c(0,0.6),ylim = c(0,1))
dev.copy(jpeg,filename=paste(path,"ћаксимальна€ св€зана€ элемента от доли удаленных верши(направ v1000",prefix,")",".jpg",sep = ""));
dev.off ();
stringname<-paste(path,"Target_BC_Table_Info_FOr_ 1000",prefix,".RData",sep="")
